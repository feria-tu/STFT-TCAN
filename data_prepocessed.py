...
# global constant
datasets = ['NAB', 'SMAP', 'SMD', 'MBA', 'SWaT', 'UCR', 'MSL', 'WADI']
# data folder
...
def load_data(dataset):
    ...
    if dataset == 'SMD':
        ...
        print('prepocessing UCR...')
        dataset_folder = 'datasets/UCR'
        file_list = os.listdir(dataset_folder)
        ...
        ls = pd.read_csv(os.path.join(dataset_folder, 'WADI_attacklabels.csv'),low_memory=False)
        train = pd.read_csv(os.path.join(dataset_folder, 'WADI_14days.csv'),low_memory=False)
        test = pd.read_csv(os.path.join(dataset_folder, 'WADI_attackdata.csv'),low_memory=False)
