...
# UCR
ucr_path = os.listdir(os.path.join(data_path,'UCR'))
os.makedirs('frequence/UCR',exist_ok=True)
...
def convert_to_freq():
    ...
    for swat in swat_path:
        ...
        if 'train' in swat:
            swat_train = np.load(os.path.join(data_path,'SWaT',swat))
            swat_train = swat_train.flatten()
            # print('swat_train:',swat_train.shape)
            # 执行STFT
            frequencies, times, Z = stft(
    ...
    for ucr in ucr_path:
        ...
        noverlap = 0  # 窗口之间的重叠量
        if 'train' in ucr:
            ucr_train = np.load(os.path.join(data_path,'UCR',ucr))
            ucr_train = ucr_train.flatten()
            # 执行STFT
            frequencies, times, Z = stft(
                ucr_train, fs=fs, nperseg=nperseg, noverlap=noverlap)
            # print('ucr_train:',ucr_train.shape)
            Z = Z.transpose()
            # print('Z:',Z.shape)
            temp = ucr.split('.')[0]
            real_Z = np.real(Z)
            np.save(os.path.join(freq_path,f'UCR/{temp}_freq'),real_Z)
        if 'test' in ucr:
            ucr_test = np.load(os.path.join(data_path,'UCR',ucr))
            ucr_test = ucr_test.flatten()
            # 执行STFT
            frequencies, times, Z = stft(
                ucr_test, fs=fs, nperseg=nperseg, noverlap=noverlap)
            Z = Z.transpose()
            temp = ucr.split('.')[0]
            real_Z = np.real(Z)
            np.save(os.path.join(freq_path,f'UCR/{temp}_freq'),real_Z)
